...
import com.guru.hibernate.dao.PersonRepository;
...
public class PersonController {
	...
	@GetMapping(value = "/persion/{id}")
	public Person getpersons(@PathVariable("id") String id){
		System.out.println(">>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<< id is "+ id);
		Integer i=Integer.valueOf(id);  
		System.out.println(">>>>>>>>>>>>>>>>>>> id <<<<<<<<<<<<<<<<<<<<< id is "+ id);
		 Person result = personRepository.findById(i);
		 System.out.println(Arrays.asList(result));
		 return result;
